...
{
    public async  Task<ResponseWrapper<TicketResponse>> Handle(GetTicketByCodeQuery request, CancellationToken cancellationToken)
    {
        ...
        if (ticket is null)
        {
            return ResponseWrapper<TicketResponse>.Fail(TicketErrors.NotFound(request.Code));
        }

        return ResponseWrapper<TicketResponse>.Success(ticket);
    }
}