...
{
    public async  Task<ResponseWrapper<TicketResponse>> Handle(GetTicketQuery request, CancellationToken cancellationToken)
    {
        ...
        if (ticket is null)
        {
            return ResponseWrapper<TicketResponse>.Fail(TicketErrors.NotFound(request.TicketId));
        }
