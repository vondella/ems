...
{
    public async  Task<ResponseWrapper> Handle(CreateOrderCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!cart.Items.Any())
        {
            return ResponseWrapper<Guid>.Fail(OrderErrors.NotFound(request.CustomerId));
        }
        ...
        foreach (CartItem cartItem in cart.Items)
        {
            ...
            {
                return ResponseWrapper<Guid>.Fail(TicketErrors.NotFound(request.CustomerId));
            }
