...
{
    public async  Task<ResponseWrapper<OrderResponse>> Handle(GetOrderQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            return ResponseWrapper<OrderResponse>.Fail(OrderErrors.NotFound(request.OrderId));
        }

        return ResponseWrapper<OrderResponse>.Success(orderResponse);
    }
}