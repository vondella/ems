...
{
    public async  Task<ResponseWrapper<PermissionsResponse>> Handle(GetUserPermissionsQuery request, CancellationToken cancellationToken)
    {
        ...
        if (!permissions.Any())
        {
            return ResponseWrapper<PermissionsResponse>.Fail(UserErrors.NotFound(request.IdentityId));
        }

        return ResponseWrapper<PermissionsResponse>.Success(new PermissionsResponse(permissions[0].UserId, permissions.Select(p => p.Permission).ToHashSet()));
    }
