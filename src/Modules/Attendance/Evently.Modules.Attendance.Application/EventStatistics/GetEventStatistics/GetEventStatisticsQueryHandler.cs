...
{
    public async Task<ResponseWrapper<EventStatisticsResponse>> Handle(GetEventStatisticsQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            return ResponseWrapper<EventStatisticsResponse>.Fail(EventErrors.NotFound(request.EventId));
        }

        return ResponseWrapper<EventStatisticsResponse>.Success(eventStatistics);
    }
}